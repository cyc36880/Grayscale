...
int flash_Sync(void)
{
    ...
    for(uint32_t i=0; i<FLASH_BUFFER_SIZE/2; i+=2)
    {
        HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, MemoryAddr+i, *((uint16_t *)(flash_buffer+i)));
    }
    //4、锁住FLASH
    HAL_FLASH_Lock();
