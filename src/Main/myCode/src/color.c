...
#define COLOR_IDENTIFY_LIGHT 700
#define SORT_SIZE 10
...
{
    get_sys_config_data(SYSCONF_MEM_LIST_COLOR_TAB, color_tab);
}
...
void color_study(uint8_t color)
{
    ...
    for (uint8_t i = 0; i < SORT_SIZE; i++)
    {
        ...
        // set_rgb_brightness(COLOR_IDENTIFY_LIGHT, COLOR_IDENTIFY_LIGHT, COLOR_IDENTIFY_LIGHT); //
...
static void rgb2hsv(lv_color_hsv_t *hsv)
{
    ...
    set_rgb_brightness(COLOR_IDENTIFY_LIGHT, 0, 0);
    HAL_Delay(10);
    for (uint8_t i = 0; i < SENSORE_NUM; i++)
    {
    ...
    set_rgb_brightness(0, COLOR_IDENTIFY_LIGHT, 0);
    HAL_Delay(10);
    for (uint8_t i = 0; i < SENSORE_NUM; i++)
    {
    ...
    set_rgb_brightness(0, 0, COLOR_IDENTIFY_LIGHT);
    HAL_Delay(10);
    for (uint8_t i = 0; i < SENSORE_NUM; i++)
    {
